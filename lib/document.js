...
if(d)console.log('m.doc.init: '+id+' populated: ', item);
...
if(d)console.log('m.doc.init: '+path+' nested-obj-no-schema: ', doc[i]);
      // assume nested object
      if (!doc[i]) doc[i] = {};
      init(self, obj[i], doc[i], path + '.');
	//} else if (schema && utils.isObject(obj[i]) &&
	//	(!obj[i].constructor || 'Object' == utils.getFunctionName(obj[i].constructor))) {
	//	console.log('m.doc.init: '+path+' schema-obj: ', doc[i]);
	//	// assume nested object
	//	if (!doc[i]) doc[i] = {};
	//	init(self, obj[i], doc[i], path + '.');
    } else {
      if (obj[i] === null) {
        ...
        if (schema) {
          try {
// ObjectId with ref to Schema + have object? Prolly populated: create sub-doc
if (schema.instance == 'ObjectID' && schema.options && schema.options.ref
	&& utils.isObject(obj[i]) && obj[i]._id) {
if(d)console.log('m.doc.init: ' + self.$__fullPath(path) + ' obj-ref: ', schema.options.ref, obj[i]);
if(d)console.log('m.doc.init: ' + self.$__fullPath(path) + ' schema: ', schema);
	//doc[i] = obj[i];
	//doc[i] = schema.cast(obj[i], self, true);
	//init(self, obj[i], doc[i], path + '.');

	if (schema.options.schema) {
		if (d)console.log('m.doc.init: ' + self.$__fullPath(path) + ' schema-to-cast-to: ', schema.options.ref, schema.options.schema);
		//doc[i] = schema.options.schema.cast(obj[i], self, true);
		if (0 && typeof window !== 'undefined') {
// Enabling this breaks doucment addition. Also population still does not work as expected :(
			// we want to instatiate a browserDocument here
			// full:    function         Document(obj, fields, skipId) {
			// browser: function         Document(obj, schema, fields, skipId, skipInit) {
			// embedded:function EmbeddedDocument(obj, parentArr, skipId, fields, index)
			var newDoc = require('./browserDocument')(obj[i], schema.options.schema);
			// set this one, so population works (does it? :P)
			newDoc.constructor.modelName = schema.options.ref;
			self.set(path, newDoc, schema.options.schema, {});
			if (d)console.log('m.doc.init: ' + self.$__fullPath(path) + ' $__.populated: ', self.$__.populated);
			if (d)console.log('m.doc.init: ' + self.$__fullPath(path) + ' schema-casted: ', schema.options.ref, newDoc);
		} else {
			// use .set() to cast to scheme
			self.set(path, obj[i], schema.options.schema, {});
			if (d)console.log('m.doc.init: ' + self.$__fullPath(path) + ' schema-casted: ', schema.options.ref, doc[i]);
		}
	}
	// do populated administration
	//self.populated(path, doc[i], schema.options);
} else {
	//if(d)console.log('m.doc.init: ' + self.$__fullPath(path) + ' schema-cast: ', schema);
			  doc[i] = schema.cast(obj[i], self, true);
}
          } catch (e) {
if(d)console.log('m.doc.init: ' + self.$__fullPath(path) + ' validation failed!', e);
...
//if(d)console.log('*** doc.set: mark '+this.$__fullPath(path)+' populated? ref '+schema.options.ref+': ', 	val.constructor.modelName);
    if (schema.options &&
        schema.options.ref &&
        val instanceof Document &&
        schema.options.ref === val.constructor.modelName) {
//if(d)
console.log('*** doc.set: mark '+this.$__fullPath(path)+' populated from '+val.constructor.modelName+': ', val._id.toString());
...
console.log('*** doc.set: ownerDoc mark '+this.$__fullPath(path)+' populated from '+val.constructor.modelName+': ', val._id.toString());
